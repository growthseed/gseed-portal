...
class ChatService {
  ...
  async getOrCreateConversation(
    ...
    console.log('[ChatService] 🔍 getOrCreateConversation:', { userId, otherUserId, projectId });
    ...
    if (findError) {
      console.error('[ChatService] ❌ Erro ao buscar conversa:', findError);
    }

    if (existing) {
      console.log('[ChatService] ✅ Conversa existente encontrada:', existing.id);
      return existing;
    }

    console.log('[ChatService] 📝 Criando nova conversa...');
    ...
    if (createError) {
      console.error('[ChatService] ❌ Erro ao criar conversa:', createError);
      throw createError;
    }
    
    console.log('[ChatService] ✅ Nova conversa criada:', newConversation.id);
  ...
  async getUserConversations(userId: string): Promise<Conversation[]> {
    try {
      console.log('[ChatService] 🔍 getUserConversations:', userId);
      ...
      console.log('[ChatService] ✅ Conversas carregadas:', data?.length || 0);
      ...
      console.error('[ChatService] 💥 Erro fatal ao buscar conversas:', error);
      return [];
    }
  }
  ...
  async getConversationMessages(conversationId: string): Promise<ChatMessage[]> {
    console.log('[ChatService] 🔍 getConversationMessages:', conversationId);
    ...
    try {
      ...
      if (userError || !user) {
        console.error('[ChatService] ❌ Usuário não autenticado!');
      ...
      console.log('[ChatService] ✅ Mensagens carregadas:', messages?.length || 0);
      
      if (messages && messages.length > 0) {
        console.log('[ChatService] 📋 Primeiras 3 mensagens:', messages.slice(0, 3));
      }
      
      return messages || [];
    } catch (error) {
      console.error('[ChatService] 💥 Erro fatal em getConversationMessages:', error);
      throw error;
    }
  }
  ...
  async sendMessage(
    ...
    try {
      ...
      console.log('[ChatService] ✅ Mensagem enviada:', message.id);
      ...
      console.error('[ChatService] 💥 Erro fatal ao enviar mensagem:', error);
      throw error;
    }
  }
  ...
  subscribeToConversation(
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload: any) => {
          ...
          if (!newRecord) {
            console.warn('[ChatService] ⚠️ Payload vazio, ignorando');
            return;
          }
          ...
          console.log('[ChatService] ✅ Mensagem enriquecida:', enrichedMessage.id);
    ...
    return () => {
      console.log('[ChatService] 🔕 Unsubscribe:', conversationId);
